...
use crate::{rdb::rdb::RedisDatabase, resp::resp::unwrap_value_to_string, store::store::Store};
use anyhow::Result;
use resp::{
...
async fn main() {
    ...
    let redis_database = flags_handler(args.into_iter().skip(1).collect()).unwrap();
        ...
        match listener.accept().await {
            Ok((stream, addr)) => {
                ...
                tokio::spawn(async move {
                    stream_handler(stream, redis_database_clone).await;
                });
            }
            Err(e) => {
...
async fn stream_handler(stream: TcpStream, mut redis_database: RedisDatabase) {
...
fn flags_handler(flags: Vec<String>) -> Result<RedisDatabase> {
    ...
    while index < flags.len() {
        ...
        match flag_name.as_str() {
            "--dir" => match flags.get(index + 1) {
                Some(dir) => {
                    redis_database.set_dir(dir.to_owned());
                }
                None => panic!("Need a directory"),
            },
            "--dbfilename" => match flags.get(index + 1) {
                Some(dir_file_name) => {
                    redis_database.set_dir_file_name(dir_file_name.to_owned());
                }
